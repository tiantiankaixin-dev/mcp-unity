...
export class UsageTracker {
  ...
  private resourceAccess: Set<string> = new Set();
  private resourceAccessTimestamps: Map<string, number> = new Map(); // Track when resources were accessed
  private toolUsage: Map<string, number> = new Map();
  private sessionStartTime: number = Date.now();
  private lastActivityTime: number = Date.now();
  private firstWorkflowWarningShown: boolean = false; // Track if we've shown the workflow warning in this session
  private logger?: Logger;
  
  // Session timeout: 1 minute of inactivity = new session (防止跨会话共享)
  private static readonly SESSION_TIMEOUT_MS = 60 * 1000;
  ...
  private static readonly RESOURCE_VALIDITY_MS = 60 * 1000; // 1 minute
   ...
   */
  recordResourceAccess(resourceUri: string) {
    const now = Date.now();
    this.lastActivityTime = now;
   ...
   */
  checkHierarchicalWorkflow(toolName: string): { allowed: boolean; warning?: string } {
    const now = Date.now();
    ...
    const hasAccessedCategory = Array.from(this.resourceAccessTimestamps.entries()).some(([uri, timestamp]) => 
      (uri.startsWith('unity://tool-names/') || uri.startsWith('unity://tool/')) && 
    ...
    const hasAccessedAnyWorkflowResource = 
      this.resourceAccess.has('unity://tool-categories') ||
...
Then use discover_and_use_batch for chained execution.`
   ...
   */
  recordPromptAccess(promptName: string) {
   ...
   */
  private suggestCategoryForTool(toolName: string): string {
