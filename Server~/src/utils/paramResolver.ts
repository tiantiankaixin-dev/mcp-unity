...
export class ParamResolver {
   ...
   */
  static resolve(
    paramsMapping: Record<string, any> | undefined,
   ...
   * @returns 解析后的实际值
   */
  private static resolveValue(value: any, context: Record<string, any>): any {
   ...
   * @returns 解析结果
   */
  private static resolveJsonPath(expression: string, context: Record<string, any>): any {
    try {
    ...
    if (typeof value === 'string' && value.startsWith('$')) {
      ...
      if (match) {
        ...
        if (!availableSteps.includes(stepName)) {
          errors.push(
            `Parameter '${path}': references unknown step '${stepName}'. ` +
