...
export class McpUnity {
    ...
    retryDelay = 1000;
    maxRetries = 3; // Maximum retries for domain reload recovery
    domainReloadRetryDelay = 2000; // Wait time between retries during domain reload
    constructor(logger) {
        this.logger = logger;
    }
    /**
    ...
    async start(clientName) {
        try {
            this.logger.info('Reading startup parameters...');
            await this.parseAndSetConfig();
            // Try to connect once on startup (don't block with multiple retries)
            try {
                this.logger.info('Attempting initial connection to Unity WebSocket...');
                ...
                this.disconnect();
                this.logger.warn('Initial Unity connection failed - will auto-connect on first tool call');
            }
        }
        catch (error) {
     ...
     */
    async sendRequestWithRetry(request, retryCount) {
     ...
     */
    sendRequestInternal(message, requestId) {
        return new Promise((resolve, reject) => {
