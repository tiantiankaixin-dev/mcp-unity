...
let RefactorScriptTool = (() => {
    let _classDecorators = [Tool({
            ...
            category: 'scripting',
            version: '1.0.0'
        }), Tags(['unity', 'scripting', 'refactoring', 'code-quality', 'cleanup']), Examples([
            {
    ...
    var RefactorScriptTool = class extends _classSuper {
        static { _classThis = this; }
        static {
            const _metadata = typeof Symbol === "function" && Symbol.metadata ? Object.create(_classSuper[Symbol.metadata] ?? null) : void 0;
            __esDecorate(null, _classDescriptor = { value: _classThis }, _classDecorators, { kind: "class", name: _classThis.name, metadata: _metadata }, null, _classExtraInitializers);
            RefactorScriptTool = _classThis = _classDescriptor.value;
        ...
        formatSuccessResponse(result) {
            const { scriptPath, operation, changesApplied = [], referencesUpdated = 0, backupPath, statistics } = result;
            let operationText = '';
            switch (operation) {
                case 'rename_symbol':
                    operationText = 'Renamed symbol';
                    break;
                case 'extract_method':
                    operationText = 'Extracted method';
                    break;
                case 'inline_variable':
                    operationText = 'Inlined variable';
                    break;
                case 'move_to_namespace':
                    operationText = 'Moved to namespace';
                    break;
                case 'organize_usings':
                    operationText = 'Organized using statements';
                    break;
                case 'format_code':
                    operationText = 'Formatted code';
                    break;
                case 'remove_unused_usings':
                    operationText = 'Removed unused usings';
                    break;
                case 'add_regions':
                    operationText = 'Added regions';
                    break;
                case 'convert_to_property':
                    operationText = 'Converted to property';
                    break;
            }
            ...
            if (statistics) {
                output += `\nStatistics:\n`;
                if (statistics.linesChanged)
                    output += `  • Lines changed: ${statistics.linesChanged}\n`;
                if (statistics.filesAffected)
                    output += `  • Files affected: ${statistics.filesAffected}\n`;
            }
    ...
    return RefactorScriptTool = _classThis;
})();
export { RefactorScriptTool };
