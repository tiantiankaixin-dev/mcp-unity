...
namespace McpUnity.Tools
{
    ...
    public class AddPackageTool : McpToolBase
    {
        ...
        // 超时时间（秒）
        private const double TIMEOUT_SECONDS = 120.0;
        ...
        public override void ExecuteAsync(JObject parameters, TaskCompletionSource<JObject> tcs)
        {
            ...
            {
                case "registry":
                    packageIdentifier = parameters["packageName"]?.ToObject<string>();
                    operation.Request = AddFromRegistry(parameters, tcs);
                    break;
                case "github":
                    packageIdentifier = parameters["repositoryUrl"]?.ToObject<string>();
                    operation.Request = AddFromGitHub(parameters, tcs);
                    break;
                case "disk":
                    packageIdentifier = parameters["path"]?.ToObject<string>();
            ...
            // 设置开始时间和包标识符
            operation.StartTime = EditorApplication.timeSinceStartup;
            operation.PackageIdentifier = packageIdentifier;
        ...
        private void CheckOperationsCompletion()
        {
            ...
            lock (_activeOperations)
            {
                ...
                for (int i = _activeOperations.Count - 1; i >= 0; i--)
                {
                    ...
                    // 检查是否超时
                    else if (currentTime - operation.StartTime > TIMEOUT_SECONDS)
                    {
                        McpLogger.LogWarning($"Package operation timed out after {TIMEOUT_SECONDS} seconds: {operation.PackageIdentifier}");
                        
                        operation.CompletionSource?.TrySetResult(new JObject
                        {
                            ["success"] = false,
                            ["type"] = "text",
                            ["message"] = $"Package operation timed out after {TIMEOUT_SECONDS} seconds. The package may still be installing in the background. Check Package Manager window for status.",
                            ["packageIdentifier"] = operation.PackageIdentifier ?? "unknown"
                        });
