...
namespace McpUnity.Resources
{
    public class GetAllToolsResource : McpResourceBase
    {
        ...
        public GetAllToolsResource(McpUnityServer server)
        {
            ...
            Uri = "unity://all-tools";
            Name = "get_all_tools";
            Description = "Get ALL Unity tools in one request (optimized for AI). Returns complete tool list with schemas for all categories.";
        }
        ...
        public override JObject Fetch(JObject parameters)
        {
            try
            {
                var allTools = new Dictionary<string, object>();
                var categorizedTools = new Dictionary<string, List<object>>();
                var toolCount = 0;
                ...
                foreach (var category in categories)
                {
                    ...
                    {
                        categorizedTools[category.name] = categoryTools;
                    }
                }

                var result = new
                {
                    _instruction = "⚠️ MANDATORY: Use discover_and_use_batch for 2+ tool calls. Chain with $.{index}.field (e.g., $.0.instanceId).",
                    _optimization = "This resource returns ALL tools in one request, eliminating the need for 23 separate category queries.",
                    totalToolCount = toolCount,
                    categoryCount = categorizedTools.Count,
